...
function guideFor(pathname: string): Guide {
    if (pathname.startsWith("/dashboard/growth")) return { title: "Growth Command guide", purpose: "This is your daily sales workspace. Start with Today, then update each lead as it moves through the pipeline.", steps: ["Contact hot leads and complete urgent tasks first.", "Move every lead to its real pipeline stage.", "Review requested calls and send the pilot payment link only after agreement.", "Add five partners each week and prepare four founder posts."], nextHref: "/ai-automation", nextLabel: "Open the Audit Page" };
    ...
    if (pathname.startsWith("/dashboard/automation")) return { title: "Automation guide", purpose: "Automation decides which campaign each lead receives based on qualification and lifecycle stage.", steps: ["Connect lead_created to the standard welcome campaign.", "Connect qualified_lead and hot_lead to dedicated sequences when available.", "Use audit_booked, proposal_sent and customer_won for later-stage follow-up.", "Save, then submit one AI audit test lead."], nextHref: "/dashboard/queue", nextLabel: "Check the Queue" };
