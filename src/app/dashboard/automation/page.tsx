...
function input(t: Theme): React.CSSProperties { return { width: "100%", padding: "9px 10px", borderRadius: 8, border: `1px solid ${t.borderLight}`, background: t.cardInner, color: t.text, fontSize: 12, outline: "none" }; }
...
export default function AutomationPage() {
    ...
    // Reuse the secret entered in Setup & Launch.
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);

    async function load(value = secret) {
        if (!value.trim()) return setMessage("Enter LANDING_PAGE_ADMIN_SECRET first.");
        sessionStorage.setItem("finmodel_admin_secret", value.trim());
        setLoading(true); setMessage("");
        const response = await fetch("/api/marketing/automation", { headers: { "x-marketing-secret": value.trim() } });
    ...
    return <main style={{ maxWidth: 1100, padding: "32px 28px 60px", fontFamily: t.font, color: t.text }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 20, flexWrap: "wrap", marginBottom: 26 }}><div><h1 style={{ fontSize: 26, margin: 0, letterSpacing: "-0.03em" }}>Automation control</h1><p style={{ color: t.textMuted, fontSize: 14, margin: "7px 0 0" }}>Connect Finasoft lead and sales events to the right email campaign.</p></div><div style={{ display: "flex", gap: 8, alignItems: "center" }}><LockKeyhole style={{ width: 15, height: 15, color: t.textMuted }} /><input type="password" value={secret} onChange={event => setSecret(event.target.value)} placeholder="Automation secret" style={{ ...input(t), width: 210 }} /><button onClick={() => load()} disabled={loading} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "9px 12px", border: 0, borderRadius: 8, background: t.accent, color: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer" }}><RefreshCw style={{ width: 13, height: 13 }} /> {loading ? "Loading…" : "Load"}</button></div></div>
        ...
        {migrationRequired && <div style={{ padding: 14, borderRadius: 10, background: t.amberSoft, color: t.textSec, fontSize: 12, marginBottom: 16 }}>Run migrations through <code>024_finasoft_growth_engine.sql</code> in Supabase before saving growth automation rules.</div>}
